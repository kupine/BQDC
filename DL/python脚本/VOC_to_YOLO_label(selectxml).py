...
def convert_annotation(image_set,image_id,classname):
    ...
    if not os.path.exists(txtpath):
        os.makedirs(txtpath)
    xmlpath='%s/coco/Annotations/%s/%s/'%(wd,image_set,classname)
    ...
    for obj in root.iter('object'):
       ...
       # if cls not in classes or int(difficult)==1:
        if cls == classname:
            copyfile('Annotations/%s/%s.xml'%(image_set, image_id), 'coco/Annotations/%s/%s/%s%s.xml'%(image_set,classname,classname,image_id))
            #in_xmlfile = open('coco/Annotations/%s/%s/%s%s.xml'%(image_set,classname,classname,image_id))
            tree=ET.parse('coco/Annotations/%s/%s/%s%s.xml'%(image_set,classname,classname,image_id))
            root = tree.getroot()
            for object in root.findall('object'):
                ...
                if cls != classname:
                    root.remove(object)
                #print (cls)
            tree.write('coco/Annotations/%s/%s/%s%s.xml'%(image_set,classname,classname,image_id))
            #in_xmlfile.close()
         ...
         #continue
            ...
            copyfile('%s/%s/%s.jpg'%(image_set,image_set,image_id), 'coco/JPEGImages/%s/%s/%s%s.jpg'%(image_set,classname,classname,image_id))
