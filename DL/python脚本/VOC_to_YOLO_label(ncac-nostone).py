...
def convert_annotation(image_set,image_id,classname):
    ...
    for obj in root.iter('object'):
       ...
       # if cls not in classes or int(difficult)==1:
        if cls == classname:
            out_file = open('ncac/labels/%s/%s/%s.txt'%(image_set,classname,image_id), 'a')
         #   continue
            ...
            i = i + 1
    if i > 0:
        copyfile('%s/%s/%s.jpg'%(image_set,image_set,image_id), 'ncac/JPEGImages/%s/%s/%s.jpg'%(image_set,classname,image_id))
        list_file.write('%s/ncac/JPEGImages/%s/%s/%s.jpg\n'%(wd,image_set,classname,image_id))
        #print (classname)
        list_file.close()
wd = getcwd()
