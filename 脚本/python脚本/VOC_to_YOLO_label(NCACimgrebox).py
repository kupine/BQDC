...
def convert_annotation(image_id,classname,cls_id):

    in_file = open('Annotations/box/%s.xml'%image_id)
    print (image_id)
    #out_file = open('ncac/%s/%s/%s.txt'%(image_set,classname,image_id), 'w')
    txtpath='%s/labels/box/'%wd
    if not os.path.exists(txtpath):
        os.makedirs(txtpath)
    jpgpath='%s/nJPEGImages/box/'%wd
    if not os.path.exists(jpgpath):
        os.makedirs(jpgpath)
    xmlpath='%s/nAnnotations/box/'%wd
    ...
    for obj in root.iter('object'):
       ...
       # if cls not in classes or int(difficult)==1:
        if cls == classname:
            out_file = open('labels/box/%s.txt'%image_id, 'a')
         #   continue
            ...
            i = i + 1
    #if i > 0:
        #copyfile('JPEGImages/box/%s.jpg'%image_id, 'nJPEGImages/box/%s.jpg'%image_id)
        #copyfile('Annotations/box/%s.xml'%image_id, 'nAnnotations/box/%s.xml'%image_id)
...
#image_ids = open('VOCdevkit/VOC%s/ImageSets/Main/%s.txt'%(year, image_set)).read().strip().split()
    label=search_xml('Annotations/box')
    sorted(label)
    ...
    for image_id in label:
        ...
        convert_annotation(newid,classname,cls_id)
    #list_file.close()
