...
for files in os.walk(fileDir_ann):
    for file in files[2]:
        ...
        classes2 = '\t\t<name>bicycle</name>\n'
        classes3 = '\t\t<name>motorbike</name>\n'
        classes4 = '\t\t<name>bus</name>\n'
        classes5 = '\t\t<name>car</name>\n'
 
        string_start = lines[0:ind_start[0]]

        if ind_end[i-1]+1 >= len(lines)-1:
            string_end = [lines[len(lines)-1]]
        else:
            string_end = lines[ind_end[i-1]+1:len(lines)]
        a = 0
        for k in range(0,len(ind_start)):
